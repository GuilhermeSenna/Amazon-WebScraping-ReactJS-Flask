...
def homepage():
    ...
    page = requests.get(
        "https://www.amazon.com.br/s?k=stephen+king",
        headers=headers)
    ...
    for produto_item in soup.findAll('div', attrs={'class': 'a-section a-spacing-base'}):
    ...
    produto_link = produto_item.find('a', attrs={'class': 'a-link-normal s-no-outline'})

    produto['link'] = f'{link_base}{produto_link["href"]}'
        
        produto['nome'] = produto_item.find('span', attrs={'class': 'a-size-base-plus a-color-base a-text-normal'}).string
